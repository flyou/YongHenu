...
public class NewsUrl {
   ...
   */
    ...
    // 新闻唯一标识id为1
    public static final String BASE_MINSHENG = "http://minsheng.henu.edu.cn/html/xyxw/1";
    ...
    // 新闻唯一标识id为8
    public static final String BASE_HUANJING = "http://cep.henu.edu.cn/main/list.php?pType=%E7%BB%BC%E5%90%88%E6%96%B0%E9%97%BB&page=";
    ...
    // 新闻唯一标识id为14
    public static final String BASE_LISHI = "http://lsxy.henu.edu.cn/a/xueyuanxinwen/list_2_";
    public static int COUNT_LISHI = 1;
    public final static String END_LISHI = ".html";
    // 历史文化学院

    // 新闻唯一标识id为15
    public static final String BASE_XINWEN = "http://xchuan.henu.edu.cn/plus/list.php?tid=14&TotalResult=160&PageNo=";
    public static int COUNT_XINWEN = 1;

    // 新闻与传播学院

    // 新闻唯一标识id为16
    public static final String BASE_SHANG = "http://sba.henu.edu.cn/Articles/33.html?&p=";
    public static int COUNT_SHANG = 1;
    ...
    // 获得新闻与传播学院的url
    public static String getXINWENUrl(int count) {

        String path;
        if (count <= 0) {
            path = BASE_XINWEN + COUNT_XINWEN;
        } else {

            path = BASE_XINWEN + count;
        }
    ...
    // 获得商学院的url
    public static String getSHANGUrl(int count) {
   //目前只有4页
        String path;
        if (count <= 0 || count > 4) {
            path = BASE_SHANG + COUNT_SHANG;
        } else {

            path = BASE_SHANG + count;
        }
    ...
    public static String getLISHIUrl(int count) {

        String path;
        if (count <= 0) {
            path = BASE_LISHI + COUNT_LISHI;
        } else {

            path = BASE_LISHI + count + END_LISHI;
        }
    ...
    public static String GetUrlByClassName(String ClassName, int count) {

        switch (ClassName) {

            case "JiaoKeYuanFragment":
                return getJKYUrl(count);
            case "MinShengFragment":
                return getMINSHENGUrl(count);
            case "FaXueFragment":
                return getFAUrl(count);

            case "WenXueFragment":
                return getWENXUEUrl(count);
            case "YaoXueFragment":
                return getYAOUrl(count);
            case "YiXueFragment":
                return getYIUrl(count);
            case "HuanJingFragment":
                return getHUANJINGUrl(count);
            case "JiSuanJiFragment":
                return getCOMPUTERUrl(count);
            case "ShengMingFragment":
                return getLIFEUrl(count);
            case "ZheXueFragment":
                return getZHEXUEUrl(count);
            case "LiShiFragment":
                return getLISHIUrl(count);
            case "XinWenFragment":
                return getXINWENUrl(count);
            case "ShangFragment":
                return  getSHANGUrl(count);
            case "SchoolMessageFragment":
                return getSchoolMessageUrl(count);
            case "EduMessageFragment":
                return  getEduNewsUrl(count);
